...
import { deleteReview } from "../../slices/reviews";
import ReviewsEditForm from "./ReviewsEditForm";

const ReviewsDetailPage = () => {
  const dispatch = useDispatch();
  const { isShow, mode, review, company } = useSelector(detailsSelector);
                    ...
                    {mode === `table` && (
                      <table className="table-auto text-sm border-gray-400 border">
                        ...
                        <tbody className="border-gray-400 border">
                          {Object.entries(review).map(([key, item], i) => (
