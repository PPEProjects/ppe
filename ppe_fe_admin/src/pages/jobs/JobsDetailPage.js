...
import { deleteJob } from "../../slices/jobs";
import JobsEditForm from "./JobsEditForm";

const JobsDetailPage = () => {
  const dispatch = useDispatch();
  const { isShow, mode, job, company } = useSelector(detailsSelector);
            ...
            <section className="w-full max-w-3xl mx-auto px-4 py-2 bg-white rounded-lg overflow-hidden shadow-md ">
              ...
              <div className="flex items-center justify-between mt-2">
                ...
                <div className="flex">
                  ...
                  <Button
                    onClick={() => dispatch(deleteJob(job))}
                    type={`button`}
                    title={`Delete`}
                    className={`bg-gray-300 text-gray-800 ml-2`}
                  />
                    ...
                    {mode === `table` && (
                      <table className="table-auto text-sm border-gray-400 border">
                        ...
                        <tbody className="border-gray-400 border">
                          {Object.entries(job).map(([key, item], i) => (
