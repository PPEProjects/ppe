...
import { deleteRelease } from "../../slices/releases";
import ReleasesEditForm from "./ReleasesEditForm";

const ReleasesDetailPage = () => {
  const dispatch = useDispatch();
  const { isShow, mode, release, project } = useSelector(detailsSelector);
            ...
            <section className="w-full max-w-3xl mx-auto px-4 py-2 bg-white rounded-lg overflow-hidden shadow-md ">
              <div className=" text-lg flex items-center justify-between ">
                <h1 className="font-semibold text-gray-700 truncate mr-8">
                  {project.name} : {release.contents[0].name}
                </h1>
                <Button
              ...
              <div className="flex items-center justify-between mt-2">
                ...
                <div className="flex">
                  ...
                  <Button
                    onClick={() => dispatch(deleteRelease(release))}
                    type={`button`}
                    title={`Delete`}
                    className={`bg-gray-300 text-gray-800 ml-2`}
                  />
                        ...
                        <div className="col-span-6">
                          <div className="w-full">
                            ...
                            <div className="mt-3">
                              ...
                              <p
                                className="text-sm text-gray-700"
                                title={release.updated_at}
                              >
                                {moment(release.updated_at).fromNow()}
                              </p>
                            </div>
                          </div>
                        </div>
                    ...
                    {mode === `table` && (
                      <table className="table-auto text-sm border-gray-400 border">
                        ...
                        <tbody className="border-gray-400 border">
                          {Object.entries(release).map(([key, item], i) => (
...
export default ReleasesDetailPage;
